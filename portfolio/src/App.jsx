...
const navItems = ['Home', 'About', 'Skills', 'Projects', 'Education', 'Experience', 'Services', 'Contact']
...
function App() {
  ...
  const filteredProjects = filter === 'All' ? portfolioData.projects : portfolioData.projects.filter((project) => project.category === filter)
  ...
  return <div className="site-shell">
    ...
    <main>
      ...
      <section id="projects" className="section-pad projects-section"><div className="section-heading row-heading"><div><p className="eyebrow">03 / Selected work</p><h2>A few things I am<br /><em>working on.</em></h2></div><div className="filters">{['All', 'Frontend', 'Backend', 'Full-Stack'].map((item) => <button className={filter === item ? 'selected' : ''} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div></div><div className="projects-grid">{filteredProjects.map((project, index) => <article className={`project-card ${project.accent}`} key={project.title}><div className="project-visual"><span>0{index + 1}</span><div className="project-mark">{project.category === 'Frontend' ? '</>' : project.category === 'Backend' ? '{ }' : '<+>'}</div><small>PROJECT IMAGE PLACEHOLDER</small></div><div className="project-info"><div className="project-title"><div><p className="project-kicker">PLACEHOLDER PROJECT</p><h3>{project.title}</h3></div><a href={portfolioData.github} target="_blank" rel="noreferrer" aria-label={`View ${project.title} on GitHub`}><Icon name="external" size={17} /></a></div><p>{project.description}</p><div className="tag-list">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></div></article>)}</div></section>
      <section id="education" className="section-pad timeline-section"><div className="section-heading"><p className="eyebrow">04 / Background</p><h2>The foundation<br /><em>so far.</em></h2></div><div className="timeline">{portfolioData.education.map((item) => <div className="timeline-item" key={item.course}><span className="timeline-dot" /><div><p className="timeline-meta">{item.year}</p><h3>{item.course}</h3><p className="timeline-place">{item.institution}</p><p>{item.description}</p></div></div>)}</div></section>
      <section id="experience" className="section-pad timeline-section experience-section"><div className="section-heading"><p className="eyebrow">05 / Experience</p><h2>Growing through<br /><em>real practice.</em></h2></div><div className="timeline">{portfolioData.experience.map((item) => <div className="timeline-item" key={item.label}><span className="timeline-dot" /><div><p className="timeline-meta">{item.year}</p><h3>{item.label}</h3><p>{item.description}</p></div></div>)}</div></section>
      <section id="services" className="section-pad services-section"><div className="section-heading centered"><p className="eyebrow">06 / What I do</p><h2>From first thought<br />to <em>working product.</em></h2></div><div className="services-grid">{services.map((service) => <article className="service-card" key={service.title}><Icon name={service.icon} /><h3>{service.title}</h3><p>{service.description}</p><span className="service-number">/</span></article>)}</div></section>
      <section id="contact" className="section-pad contact-section"><div className="contact-intro"><p className="eyebrow">07 / Say hello</p><h2>Have an idea?<br /><em>Let's talk.</em></h2><p>Whether you have a project in mind or simply want to connect, my inbox is open.</p><div className="contact-details"><a href={`mailto:${portfolioData.email}`}><Icon name="mail" /><span>{portfolioData.email}</span></a><a href={`tel:${portfolioData.phone}`}><Icon name="phone" /><span>{portfolioData.phone}</span></a><span><Icon name="pin" /><span>{portfolioData.location}</span></span></div></div><form className="contact-form" onSubmit={handleSubmit}><label>Name<input name="name" required minLength="2" placeholder="Your name" /></label><label>Email<input name="email" type="email" required placeholder="you@example.com" /></label><label>Subject<input name="subject" required placeholder="What is this about?" /></label><label>Message<textarea name="message" required minLength="10" placeholder="Tell me a little about your idea..." rows="4" /></label><button className="button button-primary" type="submit">Send Message <Icon name="arrow" size={17} /></button>{formStatus && <p className="form-status" role="status">{formStatus}</p>}<p className="form-note">Frontend demo only. Connect Formspree, EmailJS, or your own backend in this form handler to receive messages.</p></form></section>
    </main>
